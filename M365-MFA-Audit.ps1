...
# CONNECT
Write-Header "M365 MFA Audit Tool v1.1"
...
try {
    ...
    Write-Fail "Could not fetch admin roles. Continuing without admin flag."
}
...
foreach ($user in $AllUsers) {
    ...
    $Results.Add([PSCustomObject]@{
        ...
        MFAStatus   = $mfaStatus
        Methods     = if ($methodTypes.Count -gt 0) { $methodTypes -join ", " } else { "None" }
        RiskLevel   = $riskLevel
    })
}
...
$Results = $Results | Sort-Object @{
    ...
    Expression = { if ($_.IsAdmin -eq "YES") { 0 } else { 1 } }
}
...
$tableRows = foreach ($r in $Results) {
    ...
    $adminBadge = if ($r.IsAdmin -eq "YES") {
        "<span style='background:#7c3aed;color:white;padding:2px 6px;border-radius:10px;font-size:10px'>ADMIN</span>"
    } else { "" }
...
<p style='margin-top:32px;font-size:12px;color:#9ca3af'>
  Generated by M365 MFA Audit Tool - github.com/JUrica11/m365-security-toolkit<br>
